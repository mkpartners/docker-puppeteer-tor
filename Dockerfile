...
RUN apk --no-cache --no-progress upgrade && \
  apk --no-cache --no-progress add bash curl privoxy shadow tini tor tzdata make gcc g++ python3 git nodejs npm yarn libstdc++ chromium

RUN file='/etc/privoxy/config.new' && \
  cat $file && \
